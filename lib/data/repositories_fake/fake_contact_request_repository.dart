...
class FakeContactRequestRepository implements ContactRequestRepository {
  ...
  Future<ContactRequestStatus> getStatusForProfile(String profileId) async {
    ...
    if (r == null) return const ContactRequestStatus(state: ContactRequestState.none);
    ...
    if (r.state == ContactRequestState.declined) {
      return ContactRequestStatus(state: ContactRequestState.declined, requestId: r.requestId);
    }
    return ContactRequestStatus(state: ContactRequestState.pending, requestId: r.requestId);
  }

  @override
  Future<void> sendContactRequest(String profileId) async {
    await Future.delayed(const Duration(milliseconds: 80));
    final exists = _requests.any((r) => r.fromUserId == _me && r.toUserId == profileId);
    if (exists) return;
    _requests.add(_FakeRequest(
  ...
  Future<List<ReceivedContactRequest>> getReceivedContactRequests({int page = 1, int limit = 20}) async {
    ...
    final pending = _requests
        .where((r) => r.toUserId == _me && r.state == ContactRequestState.pending)
        .toList();
    final start = (page - 1) * limit;
    if (start >= pending.length) return [];
    final slice = pending.sublist(start, (start + limit).clamp(0, pending.length));
    final result = <ReceivedContactRequest>[];
    for (final r in slice) {
      final from = FakeData.allProfiles[r.fromUserId];
      if (from != null) {
        result.add(ReceivedContactRequest(
          requestId: r.requestId,
  ...
  void seedReceivedRequests() {
    ...
    for (final id in FakeData.allProfiles.keys) {
      if (id == _me) continue;
      if (i >= 2) break;
      if (_requests.any((r) => r.fromUserId == id && r.toUserId == _me)) continue;
      _requests.add(_FakeRequest(
